package com.jagerapps.trivitro_calculator.shared.domain.use_cases
...
class CalculatorFunctionsTest {
    ...
    fun test() {
        ...
        assertEquals(320, result.recommendedVitrocleanVfaLoad)
        assertEquals(400, result.recommendedSandLoad)
        assertEquals(0, result.recommendedPebble)
        assertEquals(6.4, result.fiftyBagPebble, 0.1)
        assertEquals(0.0, result.fiftyBagVitroclean, 0.1)
    }
    ...
    fun testCreateStatsByCubicFeet() {
        ...
        assertEquals(280, result.recommendedVitrocleanVfaLoad) // (5 * 80) * .7
        assertEquals(500, result.recommendedSandLoad) // 5 * 100
        assertEquals(1, result.recommendedPebble) // ((5 * .80) * .3).toInt()
        assertEquals(5.6, result.fiftyBagPebble, 0.1) // roundToNearestTenth((280 / 50))
        assertEquals(0.0, result.fiftyBagVitroclean, 0.1) // roundToNearestTenth((1.2 / 50))
    }
    ...
    fun testCreateStatsBySandNeededAbove400() {
        ...
        assertEquals(1.9, result.fiftyBagPebble, 0.1) // roundToNearestTenth((96 / 50))
        assertEquals(4.5, result.fiftyBagVitroclean, 0.1) // roundToNearestTenth((224 / 50))
